...
function Write-VersionVsixManifest {
    ...
    $document.Vsix.Identifier.Version = $version.ToString()
...
function Write-VersionAssemblyInfo {
    ...
    $assemblyInfo = Join-Path $gitHubDirectory Properties\AssemblyInfo.cs
    ...
    if ($numberOfReplacements -ne 2) {
        Die "Expected to replace the version number in 2 places in AssemblyInfo.cs (AssemblyVersion, AssemblyFileVersion) but actually replaced it in $numberOfReplacements"
    }
