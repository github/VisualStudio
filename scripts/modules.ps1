...
New-Module -ScriptBlock {
    ...
    function Build-Solution([string]$solution, [string]$target, [string]$configuration, [switch]$ForVSInstaller, [bool]$Deploy = $false) {
        ...
        Write-Host "$msbuild $solution /target:$target /property:Configuration=$configuration /p:DeployExtension=false /verbosity:minimal /fl /flp:logfile=build.log /flp:Verbosity=normal /p:VisualStudioVersion=14.0 $flag1 $flag2"
        Run-Command -Fatal { & $msbuild $solution /target:$target /property:Configuration=$configuration /p:DeployExtension=false /verbosity:minimal /fl /flp:logfile=build.log /flp:Verbosity=normal /p:VisualStudioVersion=14.0 $flag1 $flag2 }
    }
