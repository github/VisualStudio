...
namespace GitHub.Services
{
    ...
    public class GitClient : IGitClient
    {
        ...
        public GitClient()
        {
            LibGit2Sharp.Handlers.CredentialsHandler credentialProvider = (url, username, types) =>
            {
                if (url == null)
                    return null; // wondering if we should return DefaultCredentials instead

                var host = HostAddress.Create(url);
                var creds = new SimpleCredentialStore(host).GetSecureCredentials().Result;
            ...
            pushOptions = new PushOptions { CredentialsProvider = credentialProvider };
            fetchOptions = new FetchOptions { CredentialsProvider = credentialProvider };
        ...
        public GitClient(SecureCredential credentials)
        {
            LibGit2Sharp.Handlers.CredentialsHandler credentialProvider =
                (url, username, types) => new SecureUsernamePasswordCredentials
            {
